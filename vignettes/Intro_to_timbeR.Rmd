...
tree_scaling <- tree_scaling %>% 
  mutate(di_kozak.04 = predict(kozak.04))

kozak.04_rmse <- tree_scaling %>% 
  summarise(RMSE = sqrt(sum((di_kozak.04-di)^2)/mean(di_kozak.04))) %>% 
  pull(RMSE) %>% 
  round(2)

ggplot(tree_scaling, aes(x=hih))+
  geom_point(aes(y = (di_kozak.04-di)/di_kozak.04*100))+
  geom_hline(aes(yintercept = 0))+
  ...
  labs(x = 'hi / h', y = 'Residuals (%)',
       ...
       subtitle = 'Dispersion of residuals along the stem',
       caption = paste0('Root Mean Squared Error = ', kozak.04_rmse,'%'))+
  theme(plot.title.position = 'plot')
...
|------------------------------------------------------|:--------------------:|:--------------:|:-----------------:|
| Estimate the diameter at a given height              |      `poly5_di`      |     `bi_di`    |     `kozak.04_di`    |
| Estimate the height at which a given diameter occurs |      `poly5_hi`      |     `bi_hi`    |     `kozak.04_hi`    |
| Estimate the total or partial volume of the stem     |      `poly5_vol`     |    `bi_vol`    |    `kozak.04_vol`    |
| Estimate volume and quantity of logs per assortment  |     `poly5_logs`     |    `bi_logs`   |    `kozak.04_logs`   |
| Visualize the simulation of log cutting along the stem|   `poly5_logs_plot`  | `bi_logs_plot` | `kozak.04_logs_plot` |  
...
kozak.04_di(dbh, h, hi, coef_kozak.04, p = p_kozak.04)
...
ggplot(mapping=aes(x=hi))+
  ...
  geom_line(aes(y=kozak.04_di(dbh, h, hi, coef_kozak.04, p_kozak.04), linetype = 'Kozak (2004)'))+
...
kozak.04_hi(dbh, h, di, coef_kozak.04, p_kozak.04)
```
...
kozak.04_logs_plot(dbh, h, coef_kozak.04, p_kozak.04, assortments)
```
