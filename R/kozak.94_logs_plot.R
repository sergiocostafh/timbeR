...
#'
#' @details check the `kozak.94_logs` function help for more details.
#'
#' @examples
#'
#' library(dplyr)
...
#'
#' @export
kozak.94_logs_plot <- function(dbh, h, coef, p, assortments, stump_height, downgrade, broken, defect_height, lang) {
  ...
  suppressMessages(
    nlogs <-
      timbeR::kozak.94_logs(dbh, h, coef, p, assortments, stump_height, downgrade, broken, defect_height, F)
  )
  ...
  tree <- tibble::tibble(hi = seq(0, ifelse(broken,break_height,h), 0.01),
                         di = timbeR::kozak.94_di(dbh, h, seq(0, ifelse(broken,break_height,h), 0.01), coef, p))
  ...
  for (i in 1:nrow(nlogs_assortments)) {
    ...
    if (sort$Nlogs > 0) {
      hi_dpf <- timbeR::kozak.94_hi(dbh, h, sort[[2]], coef, p)
  ...
  tree_sections <- tree_sections %>%
    ...
    dplyr::mutate(desc_pos_x = dplyr::case_when(grepl(paste0(loss_label,'|',tip_label,'|',stump_label), description) ~ - timbeR::kozak.94_di(dbh,h,desc_pos_y,coef, p),
                                                TRUE ~ timbeR::kozak.94_di(dbh,h,desc_pos_y,coef, p)),
                  fontsize = dplyr::case_when(grepl(paste0(loss_label,'|',tip_label,'|',stump_label), description) ~ 3,
                                              TRUE ~ 4),
                  section = timbeR::kozak.94_di(dbh,h,hi,coef, p))
